...
void locateBall(const geometry_msgs::Pose::ConstPtr& posePtr)
{
	...
	ball.position.z=returnDistance((AaronIsCool.f*AaronIsCool.T)/(int)AaronIsCool.image.data[(int) (posePtr->position.y*640+posePtr->position.x)*4+1]);
...
AaronIsCool.image.data.resize(4*480*640);
